...
class ImageProcessing extends PApplet {
  ...
  void setup() {
    ...
    cam = new Movie(this, "D:/programmes/git info visu/Informatique-Visuelle/Game/test.avi"); //Put the video in the same directory
    cam.loop();
    ...
    daube = createGraphics(1600, 1200, P2D);
  }
  void draw() {
    ...
    List<PVector> vertices = qg.findBestQuad(edges, img3.width, img3.height, img3.width*img3.height, 100, false);

    for (PVector pv : vertices) {
      daube.ellipse(pv.x, pv.y, 20, 20);
    }

    daube.image(img2, img2.width, 0);
    daube.image(img3, 0, img3.height);
    daube.endDraw();
    image(daube, 0, 0);
